...
<head>
    ...
    <title>Mes Cours - Prof Dashboard</title>
...
<body class="bg-gray-50 flex">
    ...
    <main class="flex-1 p-8">
        <header class="flex justify-between items-center mb-10">
            ...
            <div class="text-sm text-gray-500">Connecté en tant que: <span class="font-bold text-blue-600"><?= htmlspecialchars($_SESSION['user_name'] ?? 'Prof') ?></span></div>
        </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <?php foreach ($courses as $course): ?>
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition">
                        ...
                        <a href="students_list.php?course_id=<?= $course['id'] ?>" 
                           class="flex items-center justify-center w-full py-2 bg-gray-50 hover:bg-blue-50 text-blue-600 font-semibold rounded-lg border border-blue-100 transition">
                            Voir les étudiants
                        </a>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="col-span-full py-12 text-center bg-white rounded-xl border-2 border-dashed border-gray-200">
                    <p class="text-gray-500">Vous n'avez aucun cours pour le moment.</p>
                </div>
            <?php endif; ?>
        </div>
    </main>
